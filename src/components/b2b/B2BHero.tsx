...
import { ArrowRight, Loader2, CheckCircle2, Award, Package, Headphones, TrendingUp } from "lucide-react";
...
const B2BHero = () => {
  ...
  return (
    <section id="formulario" className="relative min-h-screen flex items-center overflow-hidden">
      {/* Background */}
      <div className="absolute inset-0">
        <img src={heroImg} alt="Ciclistas KOM en ruta" className="w-full h-full object-cover" />
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          <div className="order-2 lg:order-1">
            ...
            <form onSubmit={handleSubmit} className="bg-background/10 backdrop-blur-xl border border-background/20 rounded-3xl p-6 md:p-8 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <Input placeholder="Nombre y apellido *" value={formData.nombre} onChange={(e) => updateField("nombre", e.target.value)} className={`bg-background/10 border-background/20 text-background placeholder:text-background/50 ${errors.nombre ? "border-destructive" : ""}`} />
                  </div>
                  <div>
                    <Input placeholder="Nombre de tienda" value={formData.tienda} onChange={(e) => updateField("tienda", e.target.value)} className="bg-background/10 border-background/20 text-background placeholder:text-background/50" />
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <Input placeholder="Ciudad *" value={formData.ciudad} onChange={(e) => updateField("ciudad", e.target.value)} className={`bg-background/10 border-background/20 text-background placeholder:text-background/50 ${errors.ciudad ? "border-destructive" : ""}`} />
                  <Input placeholder="País *" value={formData.pais} onChange={(e) => updateField("pais", e.target.value)} className={`bg-background/10 border-background/20 text-background placeholder:text-background/50 ${errors.pais ? "border-destructive" : ""}`} />
                </div>

                <Select value={formData.tipoNegocio} onValueChange={(v) => updateField("tipoNegocio", v)}>
                  <SelectTrigger className={`bg-background/10 border-background/20 text-background ${errors.tipoNegocio ? "border-destructive" : ""}`}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Input placeholder="WhatsApp *" value={formData.whatsapp} onChange={(e) => updateField("whatsapp", e.target.value)} className={`bg-background/10 border-background/20 text-background placeholder:text-background/50 ${errors.whatsapp ? "border-destructive" : ""}`} />
                  <Input placeholder="Correo *" type="email" value={formData.email} onChange={(e) => updateField("email", e.target.value)} className={`bg-background/10 border-background/20 text-background placeholder:text-background/50 ${errors.email ? "border-destructive" : ""}`} />
                </div>

                <Select value={formData.vendeDeportivas} onValueChange={(v) => updateField("vendeDeportivas", v)}>
                  <SelectTrigger className={`bg-background/10 border-background/20 text-background ${errors.vendeDeportivas ? "border-destructive" : ""}`}>
          ...
          <div className="order-1 lg:order-2 space-y-8">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold leading-[1.05] text-background tracking-tight">
              Conviértete en<br />
              <span className="text-accent">distribuidor</span><br />
              de KOM
            </h1>
